...
# Script to update PROJECT_STRUCTURE.md with current project structure
...
TEMP_FILE="$PROJECT_ROOT/.project_structure_temp.md"
...
# Function to generate tree-like structure using find
generate_structure() {
    local base_dir="$1"
    local prefix="$2"
    local exclude_pattern="$3"
    
    find "$base_dir" -type f -o -type d | \
...
> **Auto-generated on:** CURRENT_DATE_PLACEHOLDER  
...
sed -i "s/CURRENT_DATE_PLACEHOLDER/$CURRENT_DATE/g" "$TEMP_FILE"

# Move temp file to final location
mv "$TEMP_FILE" "$OUTPUT_FILE"

echo "✅ PROJECT_STRUCTURE.md has been updated successfully!"
